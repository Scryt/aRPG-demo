...
public class PlayerAttackEffects : MonoBehaviour
{
    ...
    public GameObject groundImpack_Prefab, kickFX_Prefab, fireTornado_Prefab, fireShield_Prefab,
        healFX_Prefab, thunderFX_Prefab;
    // Start is called before the first frame update

    void GroundImpact()
    {
        Instantiate(groundImpack_Prefab, groundImpact_Spawn.transform.position, Quaternion.identity);
    }
    ...
    void FireShield()
    {
        GameObject fireObj = Instantiate(fireShield_Prefab, fireShield_Spawn.transform.position, Quaternion.identity) 
            as GameObject;
    ...
    void ThunderAttack()
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            if (i == 0)
            {
                pos = new Vector3(transform.position.x - 4f, transform.position.y - 2,
            ...
            {
                pos = new Vector3(transform.position.x, transform.position.y - 2,
            ...
            {
                pos = new Vector3(transform.position.x + 2.5f, transform.position.y - 2,
